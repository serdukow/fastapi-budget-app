...
class OperationService:
    ...
    def get_list(self, user_id: int, kind: Optional[OperationKind] = None) -> List[tables.Operation]:
        ...
        if kind:
            query = query.filter_by(kind=kind)      # Фильтр income, outcome, salary
    ...
    def create(self, user_id: int, operation_data: OperationCreate) -> tables.Operation:
        operation = tables.Operation(
            **operation_data.dict(),
            user_id=user_id,
        )  # ** это распаковка в словарь
        self.session.add(operation)
        self.session.commit()        # коммит для того что бы записать в базу
        return operation
    ...
    def update(self, user_id: int, operation_id: int, operation_data: OperationUpdate) -> tables.Operation:
        ...
        for field, value in operation_data:
            setattr(operation, field, value)   # Обновление полей
        self.session.commit()
        return operation
